...
#include <iostream>
#include <Thread/MultiThreadOut.h>
...
using std::string;
using std::cin;
using DP::Thread::Multi_Process_Out;
...
namespace DP{
	namespace Thread{
		...
		void Manager::weitAll(const vector<int>& ids){
			lock_process();
			auto x=_all.begin();
			auto y=_th.begin();
			for (;x!=_all.end();){

...
#include <Thread/ProcessInformation.h>
#include <string>
	...
	namespace Thread{
		...
		class Thread{
			...
			public:
				...
				Thread (int id,string name):info(Process_Information(id,name)){
				...
				void run(FUNC f,ERROR er=nullfun){
						...
						try{
							...
							stat=ProcessStatus::Succes;
					#ifdef LINUX
						}catch (abi::__forced_unwind&) {  // handle pthread_cancel stack unwinding exception
							//pthread_exit(0);
							throw;
						}
					#endif

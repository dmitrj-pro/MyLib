...
extern DP::Memory::Manager memory;
...
namespace DP{
	namespace Thread{
		...
		class Manager{
			private:
				...
				inline void unlock_process(){
					lockAddProcess.unlock();
				}

			public:
				Manager(const Manager&mng):
							id(mng.id),
							_th(mng._th),
							_all(mng._all),
				...
				Manager():_th(GloBalMemoryManager.getMem<vector<thread*>>()),
	...
	//ToDo
		...
		template <typename FUNC>
		int Manager::runingWithName(FUNC manager,const string &name){
			lockAddProcess.lock();
			int is=id++;
		...
		template <typename FUNC, typename HANDLE,typename ERROR>
		int Manager::runing(FUNC manager,HANDLE trigger,const string &name,ERROR er){
			lockAddProcess.lock();
			int is=id++;
			TypeProcess* sys=new TypeProcess(is,name);
			_all.push_back(sys);
